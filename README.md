...
| [docs/spec/e2e.md](docs/spec/e2e.md)                                   | Playwright against local D1 (localhost dev sign-in)           |
...
- **Plans / 課金:** AI（相談・評価・リサーチ・ブレスト）と作成時の自動タグは premium 限定。エンタイトルメントは `PREMIUM_EMAILS`（未設定ならメンバー全員 premium）。`/api` の AI エンドポイントは未課金メンバーに **402**。**決済は未実装** — 器だけ。詳細: [docs/spec/billing.md](docs/spec/billing.md)。
...
- Sample `/api/todos` remains for template verification. `/api/ideas` mirrors that CRUD style. `/api/*` accepts the session cookie or `Authorization: Bearer <APP_API_TOKEN>` (for the planned Flutter app and scripts).
...
4. Set `GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET` / `SESSION_SECRET` with `wrangler secret put`, and add `https://<host>/api/auth/google/callback` to the Google client (do not invent client secrets)
