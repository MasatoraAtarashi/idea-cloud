...
export function ReflectionBadge({ idea }: { idea: MockIdea }) {
  ...
  return (
    <span className="font-mono text-[11px] text-muted-foreground">
      {status === "none" ? t.idea.reflection.heading : reflectionStatusLabel(t, status)}
      {snippet ? ` · ${snippet}` : ""}
    </span>
  );
}
...
export function IdeaReflectionForm({ idea, error }: { idea: MockIdea; error?: string }) {
  ...
  return (
    <section className="mt-6">
      <h3 className="text-[13.5px] font-semibold">{t.idea.reflection.heading}</h3>
      <p className="mt-1 text-[12px] text-muted-foreground">{t.idea.reflection.hint}</p>
      <fetcher.Form method="post" className="mt-2" onSubmit={hold}>
          ...
          {REFLECTION_STATUSES.map((value) => (
            ...
            >
              <input
                ...
                className="sr-only"
              />
              {reflectionStatusLabel(t, value)}
            </label>
          ))}
        </div>
        <label htmlFor="reflection-outcome" className="sr-only">
          {t.idea.reflection.outcomeLabel}
        </label>
        <input
          ...
          placeholder={t.idea.reflection.outcomePlaceholder}
          className="ui-input mt-2"
        />
        <label htmlFor="reflection-notes" className="sr-only">
          {t.idea.reflection.notesLabel}
        </label>
        <textarea
          id="reflection-notes"
          name="notes"
          ...
          placeholder={t.idea.reflection.notesPlaceholder}
          rows={3}
        ...
        <button type="submit" disabled={pending} className="ui-btn mt-2 px-3 text-[13px]">
          ...
          {pending ? t.idea.reflection.saving : t.idea.reflection.submit}
        </button>
      </fetcher.Form>
