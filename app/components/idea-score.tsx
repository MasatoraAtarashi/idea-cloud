...
export function IdeaScoreChips({ idea }: { idea: MockIdea }) {
  ...
  return (
    <span className="font-mono text-[11px] text-muted-foreground">
      {idea.humanScore ? t.idea.score.human(idea.humanScore) : null}
      {idea.humanScore && idea.aiScore ? " " : null}
      {idea.aiScore ? t.idea.score.ai(idea.aiScore) : null}
    </span>
  );
}
    ...
    <section className="mt-5 lg:mt-6">
      {compact ? null : (
        <div className="hidden lg:block">
          <h3 className="text-[13.5px] font-semibold">{t.idea.score.heading}</h3>
          <p className="mt-1 text-[12px] text-muted-foreground">{t.idea.score.hint}</p>
        </div>
      )}
        ...
        {compact ? (
          ...
          <>
            ...
            <input
              ...
              placeholder={t.idea.score.notePlaceholder}
      ...
      {compact ? null : idea.humanScoredAt ? (
        <p className="mt-1.5 hidden font-mono text-[11px] text-muted-foreground lg:block">
          {formatDateJa(t, idea.humanScoredAt)}
