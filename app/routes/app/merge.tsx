...
export default function MergePage() {
  ...
  const mergeable = useMemo(() => IDEAS.filter((idea) => idea.stage !== "archived"), []);
  const chosen = useMemo(
    () => mergeable.filter((idea) => selected.includes(idea.id)),
    [mergeable, selected],
  );
    ...
    <div className="mx-auto max-w-4xl">
      ...
      {mergeable.length === 0 ? (
        <div className="ui-panel">
          <EmptyState title="まだ融合できるアイデアはない" />
        </div>
      ) : (
        <div className="mt-1 grid gap-4 md:grid-cols-[1fr_18rem]">
