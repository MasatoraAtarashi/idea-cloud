...
| `ideas` columns    | `id`, `title`, `body`, `stage` (default `spark`), `tags` (JSON text `[]`), `created_at`, `updated_at`, research fields, and evaluation fields (`human_score` / `human_score_note` / `human_scored_at`, `ai_score` / `ai_evaluation` / `ai_evaluated_at` / `ai_evaluation_model`) |
