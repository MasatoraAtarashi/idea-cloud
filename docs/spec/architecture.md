...
| Migrations           | `todos` (template) + `ideas` + `idea_comments` + `idea_brainstorms` + `idea_chat_messages` + `saved_views` + `inspirations` (+ OGP columns in `0007_inspiration_ogp.sql`, `research_sources` in `0008_research_sources.sql`, chat in `0009_idea_chat_messages.sql`)                                                                                      |
