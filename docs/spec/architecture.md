...
| UI       | Idea detail **リサーチ** tab + list row menu POST `intent=research` (above 段階). Locked copy only for archive. `#research` is the リサーチ tab. `/app/research?from=:id` redirects there. Callout: model-only, no web search. |
