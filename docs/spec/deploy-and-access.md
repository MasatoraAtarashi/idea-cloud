...
| Confirmed `*.workers.dev` URL in this runbook                  | `https://idea-cloud.kaito-technology.workers.dev` — read from the `deploy.yml` log on 2026-09-26. **The shipped mobile client defaults to it** (`mobile/lib/api/config.dart`), so it must keep answering. Adding `routes` turns it off unless `workers_dev: true` is explicit in `wrangler.jsonc`; leaving it out took the host down on 2026-09-26 until the next deploy. |
| Custom domain                                                  | `ideacloud.polarissea.com`. Zone `polarissea.com` is active in the same account (`bf71c6a7…`), so `wrangler deploy` creates the DNS record and the certificate from the `routes` entry in `wrangler.jsonc`. Serves alongside workers.dev.                                                                                                                                 |
