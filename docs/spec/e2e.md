...
Happy paths: create idea (mobile `/app` compose or desktop header +), list, detail edit, stage advance, comment, detail **相談** tab (heading **AIと話す**, starter chips, send, Japanese fail-soft when Workers AI is down, user turn still present after reload), mobile tabs (一覧 / インスピレーション / 分析), inspiration memo, analytics charts/counts.
