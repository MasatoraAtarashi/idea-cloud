...
Detail and the list ⋯ menu have **コピー** (accessible name **説明を含めてコピー**). It writes plain text to the clipboard: title, body, then `段階:`, `カテゴリ:` when a category is set, and `タグ:` when tags exist. No idea id and no secrets. Feedback is **コピーしました**, or **コピーできませんでした** if the clipboard is blocked. There is no share-link product; the old inert **共有** control on the desktop rail is this copy action. No new API.

## Category (optional)

A **カテゴリ** is one coarse bucket per idea, separate from freeform **タグ**. It is optional on **作成** and on detail **編集**. D1 `categories` (`id`, unique `name`, `sort_order`, `created_at`) is seeded with **執筆アイデア**, **事業アイデア**, and **組織改善**. `ideas.category_id` is a nullable foreign key. The compose picker (desktop dialog and mobile **新規アイデア**) and the edit form list those rows plus **新しいカテゴリ…**, which inserts a name (max 40) or reuses an existing one. List filters add `category` (id) next to stage/tag/aged-days; named views store `categoryId`. Rows, board cards, and detail show the name. Archive behavior is unchanged. Empty category stays empty.
...
Per-idea **human 1–5** (`human_score`, optional note, timestamp) plus **AI評価**. When `TYPESAFE_API_KEY` is set, Jev scores novelty / impact / feasibility / clarity / risk, a pursue `noul`, and a next-action `choice`; code maps the composite onto 1–5 and Japanese 強み / リスク / 新規性 / 次の一手 notes. Stored Jev notes use those plain labels (進める価値 is 低め / ふつう / 高め) and still end with `スコア: N`. Otherwise Workers AI (default **標準**) writes the same headings plus `スコア: N`. Persist `ai_score`, `ai_evaluation`, `ai_evaluated_at`, `ai_evaluation_model` (`jev-latest` or a Workers AI id). Archive blocked. Compact `人N` / `AIN` chips on the list. Detail and **AI/履歴** render a **推し度** (1–5 plus a one-line meaning) and section cards. If the note text does not match those headings, the UI falls back to the stored pre-wrap text. Raw model ids and axis decimals stay out of that primary view; a short preset or Jev label can sit on a secondary line. Not Relic multi-axis / S/A/B scoring.

**作成** (form action and `POST /api/ideas`) schedules that same evaluation with `waitUntil` after the row is inserted, so the response is not held for the model. **アーカイブ** is skipped. If Jev and Workers AI both fail, the idea still exists and the create response stays successful. MCP `create_idea` does not auto-tag or auto-evaluate.

## Discuss (per-idea chat)

Detail tab **相談** (`#discuss`), heading **AIと話す**. The user can ask freeform questions about that one idea (for example an LP direction or legal risk). Turns persist in D1 `idea_chat_messages` (`idea_id`, `role` `user` | `assistant`, `body`, nullable `model`, `created_at`), oldest first, and survive reload. Each reply uses Workers AI (default preset **標準**, same allowlist as research). The prompt includes the idea title, body, stage, tags, a shortened latest AI評価 (decimals softened), and a few recent comments. It does not call Jev and does not invent citations. Legal-risk questions get a short Japanese disclaimer that the reply is a thinking aid, not legal advice. **アーカイブ** is blocked (**アーカイブでは相談できません**). Failures return a Japanese error; the user turn is still stored. Starter chips (**LPにするなら** / **法的リスクは？** / **次の一手は？** / **競合との差別化**) send that prompt. The desktop rail, mobile detail AI swipe, and list ⋯ menu link here. Pending UI shows on the click tick. Tests stub `setTestAiRun`. Remote MCP does not add a discuss tool; see [mcp.md](./mcp.md).
...
D1 `inspirations` (`title`, nullable `url`, `memo`, optional tags, plus cached Open Graph fields). `/app/inspirations` is a **mood-board gallery** (2-col mobile, masonry-like on desktop), not a table, and a **first-class nav destination** (desktop sidebar + mobile インスピ tab). Cards show `og:image` when present, otherwise a domain-glyph fallback. Detail shows the same preview prominently and can **再取得**. Create is dump-and-go: a URL alone is enough, and the title is optional. The URL field is plain text (`inputmode=url`, not `type=url`) so a mobile paste is not rejected by the browser. `http://` and `https://` are both saved; surrounding whitespace, newlines, and zero-width characters are stripped; a bare host or host/path is stored as `https://…`. When the title is empty and a URL is present, the card title becomes the fetched page title, otherwise a path slug, the hostname, or 「無題」. Saving a URL from the gallery or API fetches `og:title` / `og:description` / `og:image` / `og:site_name` (twitter:image fallback) with timeout, size cap, and SSRF blocks; fetch failure stores `og_status=failed` and does **not** fail the save. Errors name the real failure (empty input, malformed URL, or a non-http(s) scheme). Images are hotlinked (`<img loading=lazy referrerpolicy=no-referrer>`); no R2 upload. **AIブレスト** still creates an idea; seed text may include `og_title`. Creating or editing an idea whose body contains http(s) URLs also upserts those URLs here (max 5, hostname/context title, fail-soft, no page fetch at save time). Auto-imported rows use the gallery glyph fallback until **再取得** or a later URL save fills OGP.
